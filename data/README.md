...
## 关于实验 11 的全幅填充（合成占位数据）

实验 11 为了做"输入覆盖全球"的实装测试，对候选区以外的像元填入了**合成占位数据**：

- 连续参数取参数域"最差端"5% 区间内的值 + 固定种子平滑噪声，评分 ∈ [0, 0.05]；
- 分类参数取评分最低档位；
- 合成像元在 `outputs/rasters/bennu_fill_mask.tiff` 中标记为 **0**，候选区标记为 1。

这些填充值**不是观测数据**，仅用于让产品无空洞、跑通全幅流程；任何引用该栅格的结果
都必须先用掩膜区分候选区与填充区。种子固定为 20260920，可复现。
