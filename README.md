...
- 连续参数：取值限制在参数域"最差端"的 5% 区间内并叠加固定种子的平滑噪声，
  评分 ∈ [0, 0.05]；
- 分类参数：取评分最低的档位（分类评分是离散的，无法做到"趋近于 0"）；
- 因此**填充区评分恒 ≤ 候选区最小值**（实测填充区 ISVM ≤ 0.0710，
  候选区 ≥ 0.5772，相差 0.506），既不改变候选区结果，又让二者差别最大；
...
**实装测试结果**（2026-09-20）：全幅 4.93 M 像元一次跑通，全流程 36.4 s
（填充+评分 1.1 s，栅格写出 9.7 s），全部栅格无 nodata。候选区数值与改动前**逐项相等**，
用户可据此确认填充未影响结果。另有两个工程后果值得注意：全幅 float32 栅格体积显著增大
（10 波段贡献度 73 MB、占比 116 MB，合计 207 MB），其中占比栅格超过 GitHub 单文件
100 MB 上限，因此这两个大栅格已从版本库移除（本地仍在，可 36 s 重跑生成），
入库的是成品图、ISVM 单波段、掩膜与 CSV。
